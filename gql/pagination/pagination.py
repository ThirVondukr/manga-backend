...
class CursorPaginator(Generic[TModel, TType]):
    def __init__(self, type_cls: Type[FromORM[TModel, TType]], cursor_func: Callable[[TType], str]):
    ...
    def parse_sequence(self, sequence: Sequence[TModel], first: int) -> Connection[TType]:
        ...
        page_info = PageInfo(has_next_page=has_next_page, end_cursor=self.cursor_func(nodes[-1]))
